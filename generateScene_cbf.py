...
RANDOM_AREA_X = 5.0   
RANDOM_AREA_Y = 5.0   
CUSTOM_POSITIONS = [(0, 0, 0), (1, 1, 45), (2, 0, 90)]

# --- Visual Settings ---
HIGHLIGHT_LEADER = True
LEADER_COLOR_RGBA = "0.9 0.1 0.1 1.0"  # Red Leader
OBSTACLE_COLOR = "0.2 0.3 0.4 1.0"     # Blue/Grey Obstacle

# --- OBSTACLE & BOUNDARY SETTINGS ---
ADD_OBSTACLE = True            # <--- ENABLED
OBSTACLE_POS = [3.0, 0.0, 0] # X=5 meters ahead
OBSTACLE_RADIUS = 0.6          # Physical collision size
OBSTACLE_HEIGHT = 1.2
SAFE_BUFFER = 1.5              # The "Danger Zone" radius (1.5m extra)
  ...
  <default>
    <default class="go2">
      ...
      <default class="collision"><geom group="3"/>
        <default class="foot"><geom size="0.022" pos="-0.002 0 -0.213" priority="1" condim="6" friction="0.8 0.02 0.01"/></default>
      </default>
    </default>
  </default>
"""
...
SHARED_ASSETS = f"""
  <asset>
    ...
    <material name="mat_boundary" rgba="1.0 0.0 0.0 0.3"       shininess="0"/>
...
OBSTACLE_TEMPLATE = """
    <body name="obstacle" pos="{x} {y} {z}">
        ...
        <geom type="cylinder" size="{bound_rad} {height}" material="mat_boundary" contype="0" conaffinity="0"/>
    </body>
"""
...
def get_positions():
    ...
    if FORMATION_TYPE == "LINE":
        ...
        for i in range(NUM_ROBOTS):
            ...
            if FACING_MODE == "CENTER": yaw = angle_deg + 180
            elif FACING_MODE == "OUTWARD": yaw = angle_deg
            elif FACING_MODE == "TANGENT": yaw = angle_deg + 90
        ...
        for pos in CUSTOM_POSITIONS:
            if len(pos) == 3: positions.append((pos[0], pos[1], 0.445, pos[2]))
            elif len(pos) == 4: positions.append(pos)
                
    if not positions: positions = [(0,0,0.45,0)]
    return positions
...
def generate_files():
    ...
    for i, (x, y, z, yaw) in enumerate(positions):
        ...
        bodies_xml += ROBOT_BODY_TEMPLATE.format(i=i, pos_str=pos_str, euler_str=euler_str, mat=current_materials)
    ...
    if ADD_OBSTACLE:
        obstacle_xml = OBSTACLE_TEMPLATE.format(
            x=OBSTACLE_POS[0], y=OBSTACLE_POS[1], z=OBSTACLE_POS[2], 
            radius=OBSTACLE_RADIUS,
            bound_rad=OBSTACLE_RADIUS + SAFE_BUFFER, # <--- Visual Boundary
  ...
  <actuator>{actuators_xml}</actuator>
  <sensor>{sensors_xml}</sensor>
</mujoco>
"""
